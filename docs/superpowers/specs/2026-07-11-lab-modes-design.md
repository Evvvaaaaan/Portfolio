...
| 모드 UI 카피 | 영어 고정 (터미널·DevTools·스피드런 미학과 일치). 단 Terminal이 보여주는 콘텐츠는 LangContext 현재 언어를 재사용 | i18n 4개 언어 파일 수정 회피, 콘텐츠 중복 방지 |
| 진입점 | Navbar 데스크톱 네비의 "Mode" 항목 1개 (v1은 데스크톱 우선, 모바일 메뉴 노출은 2차) | 핵심 제약(콘텐츠 방해 금지) 준수 |
...
### 3. Inspect (해부 투어)
- 가짜 DevTools 패널이 우측에 도킹. "Prev/Next" 버튼으로 순서대로 도는
  가이드 투어 — 현재 스텝의 요소가 DevTools처럼 하이라이트됨.
  (호버로 임의 요소 선택은 v1 제외 — 호버 시 자동 스크롤과 충돌)
- CSS 덤프 대신 큐레이션된 주석("이 섹션은 왜 이렇게 만들었나",
  기술 노트)을 보여줌. 요소별 주석은 정적 데이터 파일로 관리.

### 4. Speedrun (스피드런)
- 우측에 타이머 + 스플릿 패널 (LiveSplit 스타일).
- 미션: 5개 섹션(#home, #about, #projects, #skills, #contact) 방문
  스플릿을 최단 시간에 완수. (프로젝트 열기·이력서 모달 미션은 제외 —
  모드가 메인 페이지 스코프라 라우팅 이동이 런을 끊기 때문)
- 섹션 방문 감지는 IntersectionObserver.
- 타이머 표시는 rAF로 DOM에 직접 기록 (프레임마다 React 리렌더 금지).
