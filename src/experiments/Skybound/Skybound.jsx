...
import { GATES, ISLANDS, STEP, createFlight, stepFlight, flightTelemetry, clamp } from './game.js'
...
const reasons = { water: '수면과 충돌했습니다. 고도를 조금 더 높여 보세요.', terrain: '지형과 충돌했습니다. 산을 피해 선회하거나 상승하세요.', boundary: '비행 구역을 벗어났습니다. 섬 주변의 항로로 돌아와 주세요.', timeout: '제한 시간이 끝났습니다. 다음 비행에서 코스를 이어가 보세요.' }
const formatTime = (seconds) => `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(Math.floor(seconds % 60)).padStart(2, '0')}`
const VIEWS = { cockpit: '일인칭 조종석', chase: '추적 시점', nose: '기수 시점' }

function savedBest() { try { const value = Number(localStorage.getItem('skybound-best')); return value > 0 && Number.isFinite(value) ? value : null } catch { return null } }
...
function FlightMap({ ui }) {
  const x = (n) => (n + 500) / 2000 * 160
  const y = (n) => (n + 1250) / 1900 * 160
  return <div className="sb-map"><div className="sb-map-title"><span>FLIGHT PLAN</span><span>N ↑</span></div>
    <svg viewBox="0 0 160 160" role="img" aria-label="섬과 체크포인트, 현재 비행기 위치를 표시한 지도">
      ...
      {ISLANDS.map((island, i) => <ellipse key={i} cx={x(island.x)} cy={y(island.z)} rx={island.rx / 2000 * 128} ry={island.rz / 1900 * 128} fill="#93b5a27a" />)}
    ...
    <span className="sb-map-caption">THE AURELIA ISLANDS</span>
  </div>
}
...
export default function Skybound() {
  ...
  useEffect(() => {
    let game = createFlight(choice.current)
    let scene
    ...
    const command = (action) => {
      ...
      if (action === 'start' || action === 'restart') {
        game = createFlight(choice.current)
        game.phase = 'playing'
        clear()
        scene.reset()
      ...
      if (game.phase === 'playing') {
        ...
        if (game.phase === 'won') {
          ...
          if (!saved || game.elapsed < saved) {
            try { localStorage.setItem('skybound-best', String(game.elapsed)) } catch { /* Private browsing: keep the result for this visit. */ }
  ...
  return <main className={`skybound${calm ? ' sb-calm' : ''}`} ref={rootRef} tabIndex={0} aria-label="Skybound 비행 게임" data-lenis-prevent data-phase={ui.phase} data-gate={ui.gate} data-mode={ui.mode} data-view={view}>
    ...
    <header className="sb-header"><span className="sb-brand"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 2 2 8 8 4v2l-8-2-1 6 3 2H8l3-2-1-6-8 2v-2l8-4z" fill="currentColor" /></svg>SKYBOUND<span>FLIGHT CLUB</span></span>
      <div className="sb-header-tools"><button type="button" aria-label="엔진 소리" aria-pressed={sound} onClick={() => { const next = !sound; setSound(next); apiRef.current?.sound(next); rootRef.current?.focus({ preventScroll: true }) }}>{sound ? 'SOUND ON' : 'SOUND OFF'}</button><button type="button" aria-label="카메라 움직임 줄이기" aria-pressed={calm} onClick={() => { setCalm(!calm); apiRef.current?.calm(!calm); rootRef.current?.focus({ preventScroll: true }) }}>CALM {calm ? 'ON' : 'OFF'}</button>{active && <button type="button" className="sb-pause-button" aria-label="비행 일시정지" onClick={() => command('pause')}>Ⅱ</button>}</div>
    </header>

    {!overlay && <>
      <div className="sb-mission"><span className="sb-overline">{mode === 'course' ? `CHECKPOINT ${String(Math.min(8, ui.gate + 1)).padStart(2, '0')} / 08` : 'FREE FLIGHT'}</span><h2>{mode === 'course' ? GATES[ui.gate]?.name : '당신만의 항로'}</h2><p>{mode === 'course' ? `다음 링까지 ${ui.distance} m` : '서두르지 말고 섬을 둘러보세요'}</p></div>
      ...
      {ui.notice && <div className="sb-notice" role="status">{ui.notice}<span>KEEP YOUR LINE.</span></div>}
      {ui.clearance < 32 && <div className="sb-warning" role="status">↑ 고도가 낮습니다 — 상승하세요</div>}
      {Math.hypot(ui.x, ui.z) > 2400 && <div className="sb-warning" role="status">비행 구역 경계입니다 — 섬으로 선회하세요</div>}
    </>}
    ...
    {!overlay && <><button type="button" className="sb-view-button" onClick={() => command('view')} aria-label="카메라 시점 전환"><span>⌖</span>{VIEWS[view]}<kbd>C</kbd></button>
      {view === 'cockpit' && <div className="sb-pilot-label"><span>PILOT VIEW</span><i /> SB—01 <b>{String(ui.heading).padStart(3, '0')}°</b></div>}
    ...
    {overlay && <div className={`sb-overlay ${ui.phase === 'ready' && !failed ? 'sb-intro-overlay' : ''}`}><section className="sb-card">
      ...
      : ui.phase === 'ready' ? <>
        ...
        <p className="sb-intro-copy">푸른 섬 사이로, 구름 아래로.<br />조종간을 잡고 당신만의 비행을 시작하세요.</p>
        <div className="sb-mode-select" role="group" aria-label="비행 모드"><button type="button" aria-pressed={mode === 'course'} onClick={() => chooseMode('course')}><span>01 <strong>체크포인트</strong></span><small>8개의 링 · 2분 30초</small></button><button type="button" aria-pressed={mode === 'free'} onClick={() => chooseMode('free')}><span>02 <strong>자유 비행</strong></span><small>시간 제한 없는 탐험</small></button></div>
      ...
      </> : ui.phase === 'paused' ? <><span className="sb-overline">FLIGHT ON HOLD</span><h1>잠시, 숨을 고르고.</h1><p>비행과 타이머가 멈췄습니다. 탭을 떠나거나 창의 포커스를 잃어도 자동으로 일시정지됩니다.</p><div className="sb-pause-controls"><span>W / ↑ 상승 · S / ↓ 하강</span><span>A D / ← → 선회 · Shift / + 가속</span><span>Space 감속 · − 추력 감소 · C 시점 전환</span><span>P / Esc 일시정지 · R 재시작</span></div><button type="button" className="sb-start" onClick={() => command('resume')}>비행 계속하기 <span>↗</span></button><button type="button" className="sb-secondary" onClick={() => command('restart')}>처음부터 다시 비행</button></>
      : <><span className="sb-overline">{ui.phase === 'won' ? 'ROUTE COMPLETE' : 'FLIGHT ENDED'}</span><h1>{ui.phase === 'won' ? '멋진 비행이었어요.' : '다시 날아볼까요?'}</h1><p>{ui.phase === 'won' ? '오렐리아의 여덟 항로를 모두 통과했습니다. 하늘에는 언제나 새로운 길이 있습니다.' : reasons[ui.reason]}</p><div className="sb-results"><div><span>SCORE</span><strong>{ui.score}</strong></div><div><span>FLIGHT TIME</span><strong>{formatTime(ui.elapsed)}</strong></div><div><span>CHECKPOINTS</span><strong>{ui.gate}<small>/ 8</small></strong></div></div>{best && <p className="sb-best">PERSONAL BEST <span>{formatTime(best)}</span></p>}<button type="button" className="sb-start" onClick={() => command('restart')}>다시 비행하기 <span>↗</span></button></>}
      <a className="sb-asset-credits" href="/skybound/ATTRIBUTION.md" target="_blank" rel="noreferrer">3D 모델 · 환경 에셋 크레딧 ↗</a>
    </section>{ui.phase === 'ready' && !failed && <div className="sb-aircraft-label"><span>CESSNA 172</span><strong>THE COASTAL EXPLORER</strong><p>Single engine. Endless possibilities.</p><i /></div>}</div>}
    <footer className="sb-footer"><span>AURELIA ISLANDS <i> / </i> A FICTIONAL ARCHIPELAGO</span><span>SKYBOUND — A FLIGHT EXPERIMENT</span></footer>
  </main>
}
